...
function verificationVide(){
  ...
  if (nomPrenom.length == 0 ||email.length == 0 ||telephone.length == 0 || password.length == 0 ||confirm_password.length == 0 || poste == null || sexe == null || structure == null) {
...
function afficherLesOptionsDAdministrations(){
    var arr = JSON.parse(localStorage.getItem("BDadministration"));                
    var out = "";
    var i; 
    for(i = 0; i < arr.length; i++) { 
      out = '<option value="'+arr[i].id_administration+'">'+arr[i].denomination_administration+'</option>';
      $('#listadministrations').append(out); 
    }
}
...
function afficherLesOptionsDeStructureModal(k){
  var arr = JSON.parse(localStorage.getItem("BDstructure"));                
    var out = "";
    var i; 
    for(i = 0; i < arr.length; i++) { 
      document.getElementById('structure'+k+'').innerHTML += '<option value="'+arr[i].id_structure+'">'+arr[i].intitule_structure+'</option>'; 
    }
}

function modal(k){
  afficherLesOptionsDePostesModal(k);
  afficherLesOptionsDeStructureModal(k);
}
...
function supprimerResponsable(k){
  ...
  xhttp.onreadystatechange = function() {
    ...
    if (this.responseText==3) { $('#responsable_'+k).hide(1000); listeUtilisateurs();}
    }
  };
  ...
  if (localStorage.getItem('responsableamodifier') == "" || localStorage.getItem('responsableamodifier') == null) {
          ...
          xhttp.onreadystatechange = function() {
            ...
            if (this.responseText==2) { 
              dernierEnregistrementResponsable();listeUtilisateurs();localStorage.setItem('selectedadministration',"");
